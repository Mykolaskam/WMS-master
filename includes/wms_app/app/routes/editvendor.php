...
$app->get('/deletevendor', function ($request, $response, $args) {
    ...
    if ($wrapper_sql->session_var_exists(session_id())) {
        ...
        $wrapper_sql->delete_vendor_var($session_wrapper->get_session('vendor_id'));

        return $this->response->withRedirect('/wms/index.php/vendors');

    }

})->setName('deletevendor');


$app->get('/editvendor/{id}', function ($request, $response, $args) {
    ...
    $session_wrapper->set_session('vendor_id', $vendor_id);

    if ($wrapper_sql->session_var_exists(session_id())) {

        $the_vendor = $wrapper_sql->get_vendor_by_id_var($vendor_id);
        ...
        return $this->view->render($response,
            ...
            [
                ...
                'action_update_vendor' => '/wms/index.php/editvendor',
            ]);
...
$app->post('/editvendor', function (Request $request, Response $response) {
    ...
    if ($wrapper_sql->session_var_exists(session_id())) {
        ...
        $wrapper_sql->update_vendor_var(
            ...
            $sanitised_notes,
            $session_wrapper->get_session('vendor_id'));
